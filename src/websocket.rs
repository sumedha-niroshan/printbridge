...
async fn handle_connection(
    ...
    let ws_stream = accept_hdr_async(tls_stream, |req: &Request, response: Response| {
        // Origin check
...
async fn process_message(text: &str) -> OutgoingMessage {
    let incoming: IncomingMessage = match serde_json::from_str(text) {
        Ok(m) => m,
        Err(e) => {
            return OutgoingMessage::err("unknown", format!("Invalid message: {}", e));
        }
    };
    ...
    match incoming {
        ...
        IncomingMessage::Print { id, payload } => {
            ...
            let bytes = match base64::engine::general_purpose::STANDARD.decode(&payload.data) {
                Ok(b) => b,
                Err(e) => {
                    // Try treating as raw UTF-8 text
                    match payload.print_type {
