...
#[cfg(windows)]
static CONFIRM_QUIT: AtomicBool = AtomicBool::new(false);
...
impl PxlApp {
    pub fn new(
        ...
        #[cfg(windows)]
        let (tray_icon_opt, show_menu_id_opt, quit_menu_id_opt) = {
            use tray_icon::{
            ...
            // Create tray icon (32x32 green circle status indicator)
            let width = 32;
            let height = 32;
            ...
            let tray = if let Ok(icon) = tray_icon::Icon::from_rgba(rgba, width as u32, height as u32) {
                TrayIconBuilder::new()
            ...
            (tray, Some(show_id), Some(quit_id))
        };
                ...
                loop {
                    ...
                    while let Ok(event) = tray_icon::menu::MenuEvent::receiver().try_recv() {
                        ...
                        if let Some(ref q_id) = quit_id {
                            if event.id == *q_id {
                                CONFIRM_QUIT.store(true, Ordering::SeqCst);
...
impl eframe::App for PxlApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Hide/minimize on close instead of quitting
